...
export const useMarkets = (options?: UseMarketsOptions): UseMarketsReturn => {
  ...
  // Query keys
  const MARKETS_QUERY_KEY = (
    filters: MarketFilters,
    page: number,
    limit: number
  ) => ['markets', filters, page, limit];

  const hasSort = Boolean(currentFilters.sortBy);
  ...
  const marketsQuery = useQuery({
    ...
    enabled: autoLoad && !infiniteScroll,
  ...
  // Infinite scroll markets query (works with server-side sorting)
  const infiniteMarketsQuery = useInfiniteQuery({
    ...
    getNextPageParam: (lastPage: any) =>
      (lastPage as any).hasMore ? (lastPage as any).nextPage : undefined,
  });

  // Get total from the first page of infinite query results
  const infiniteTotal = infiniteMarketsQuery.data?.pages?.[0]?.total ?? 0;

  // Mark initial load complete after first successful fetch
  useEffect(() => {
    const marketsData = infiniteScroll
      ? infiniteMarketsQuery.data?.pages.flatMap((p: any) => p.data)
      : marketsQuery.data;
    const marketsLength = marketsData?.length ?? 0;
    if (
      !initialLoadComplete &&
      !marketsQuery.isLoading &&
      !infiniteMarketsQuery.isLoading &&
      marketsLength > 0
    ) {
  ...
  const trackMarket = useCallback(
    async (marketId: string, status?: string, attendingDates?: string[]) => {
      return new Promise<void>((resolve, reject) => {
        trackMutation.mutate(
          { marketId, status, attendingDates } as {
            marketId: string;
            status?: string;
            attendingDates?: string[];
          },
  ...
  const untrackMarket = useCallback(
  ...
  const trackedMarketsQuery = useTrackedMarketsQuery(
    initialLoadComplete && user?.id ? user.id : undefined
  );
  const trackedMarketIds =
  ...
  const getMarketById = useCallback(
    ...
    [marketsQuery.data, infiniteMarketsQuery.data, infiniteScroll]
  );

  // Is market tracked helper
  const isMarketTracked = useCallback(
    (marketId: string) => {
  ...
  const isInitialLoading = infiniteScroll
    ? infiniteMarketsQuery.isLoading
    : marketsQuery.isLoading;

  const isFetchingMore = infiniteScroll
    ? infiniteMarketsQuery.isFetching &&
      !infiniteMarketsQuery.isLoading &&
      (infiniteMarketsQuery.data?.pages.length || 0) > 0
  ...
  // isFetching - true when any fetch is in progress (for blocking duplicate infinite scroll triggers)
  const isFetching = infiniteScroll
    ? infiniteMarketsQuery.isFetching
    : marketsQuery.isFetching;
  
  // isSearching - only true when actively searching with a search term
  const isSearching = Boolean(currentFilters.search) && isFetching;
    
  const isTracking = trackMutation.isPending || untrackMutation.isPending;
  const error =
    marketsQuery.error?.message ||
    infiniteMarketsQuery.error?.message ||
    null;

  const rawMarkets: Market[] = infiniteScroll
    ? infiniteMarketsQuery.data?.pages.flatMap((page: any) => page.data) || []
    : marketsQuery.data || [];

  // Apply client-side sorting for date-based sorting (date-oldest, date-newest)
  // These need client-side sorting by schedule date
  const sortedMarkets = useMemo<Market[]>(() => {
    if (!hasSort || !rawMarkets || !Array.isArray(rawMarkets)) {
      return rawMarkets || [];
    }

    const { sortBy } = currentFilters;

    // Date-based sorting needs client-side sorting
    const needsClientSideSort =
      sortBy === 'date-oldest' || sortBy === 'date-newest';
  ...
  }, [rawMarkets, hasSort, currentFilters.sortBy]);

  // Use sorted markets for date-based sorting, otherwise use rawMarkets (server-side sorted)
  ...
  return {
    ...
    currentPage,
    totalPages,
    total: infiniteScroll ? infiniteTotal : total,
    hasMore,
    // Infinite scroll works with server-side sorting
    hasNextPage: infiniteScroll
      ? infiniteMarketsQuery.hasNextPage
      : currentPage < totalPages,
    fetchNextPage: infiniteScroll
      ? infiniteMarketsQuery.fetchNextPage
      : nextPage,
...
const TRACKED_MARKETS_QUERY_KEY = (userId?: string) => [
  'trackedMarkets',
  userId || '',
];
...
const useTrackedMarketsQuery = (userId: string | undefined) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      console.log(
        '[useTrackedMarketsQuery] Received tracking array:',
        trackingArray
      );
  ...
  return useMutation({
    ...
    onMutate: async ({ marketId, status }) => {
      ...
      queryClient.setQueryData(queryKey, (old: any) => {
        ...
        const hasMarket = old.some(
          (m: any) => m.market?.id === marketId || m.marketId === marketId
        );
    ...
    onError: (err, _variables, context: any) => {
      ...
      if (context?.previousData !== undefined && userId) {
        queryClient.setQueryData(
          TRACKED_MARKETS_QUERY_KEY(userId),
...
export const useTrackedMarkets = (): UseTrackedMarketsReturn => {
  ...
  const handleTrackMarket = useCallback(
    (marketId: string, status?: string, attendingDates?: string[]) => {
      trackMutation.mutate({ marketId, status, attendingDates } as {
        marketId: string;
        status?: string;
        attendingDates?: string[];
      });
